<template>
  <div ref="container" class="fixed top-0 left-0 w-screen h-screen -z-10">
...
class CustomShaderMaterial extends THREE.ShaderMaterial {
  constructor(){
    super({
      uniforms:{
        time:{value:0},
        color1:{value:new THREE.Color('#0A0F2C').multiplyScalar(1.2)},
        color2:{value:new THREE.Color('#2D2A4A').multiplyScalar(0.8)},
        color3:{value:new THREE.Color('#D7D7D7').multiplyScalar(0.6)},
  ...
  float movement = time*0.06;
  float wave = fbm(uv*2.0 + movement);

  //smooth transition and dynamic flow
  float dynamicWave = sin(uv.y*8.0+time*1.5)*0.1+wave;
  dynamicWave = smoothstep(0.2,0.8, dynamicWave);

  vec3 color = mix(color1,color2,dynamicWave);
  float hightlight = smoothstep(0.6,1.0,dynamicWave);
  vec3 finalColor = mix(color,color3,hightlight*0.5);
