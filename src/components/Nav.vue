<template>
  ...
  <div class="sticky top-0 left-0 z-30 bg-black/40 backdrop-blur-md w-full h-20 flex items-center justify-center px-6 mb-4">
    <!-- Hamburger Button (absolute left) -->
    <button
      v-if="!isOpen"
      @click="isOpen = !isOpen"
      aria-label="Open menu"
      class="absolute left-6 top-1/2 -translate-y-1/2 bg-white/10 backdrop-blur-md w-12 h-12 rounded-full flex items-center justify-center shadow-lg hover:scale-105 transition-transform duration-200 focus:outline-none focus:ring-2 focus:ring-primary z-40 border border-white/30"
    >
    ...
    class="fixed inset-0 bg-black bg-opacity-30 flex flex-col items-center justify-center z-50 min-h-screen min-w-full"
      >
        ...
        <button
          ...
          class="absolute top-5 left-5 bg-white bg-opacity-20 rounded-full p-2 hover:bg-opacity-40 transition"
        >
          <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <nav class="flex flex-col space-y-8">
          <ul class="space-y-12">
            <li
              ...
              :class="['text-white text-2xl font-bold cursor-pointer menu-item list-none', { visible: i < revealedCount, 'menu-item-animate': i < revealedCount }]"
          @click="item.toLowerCase().includes('home') ? ($router.push('/'), isOpen = false) : (item.toLowerCase().includes('services') ? ($router.push('/services'), isOpen = false) : (item.toLowerCase().includes('skills') ? ($router.push('/skills'), isOpen = false) : (item.toLowerCase().includes('projects') ? ($router.push('/projects'), isOpen = false) : (item.toLowerCase().includes('contact') ? ($router.push('/contact'), isOpen = false) : isOpen = false))))"
            >
